"SELECT A.COL_IndentNo , A.COL_IndentDate , A.COL_RegistrationId , A.COL_EncounterId , A.COL_AdvisingDoctorId ,
D.COL_FirstName As AdvisingDoctorName ,
B.COL_ItemId , C.COL_ItemName , B.COL_RouteId  , B.COL_StrengthId , B.COL_FrequencyId , B.COL_PrescriptionDetail  ,
IsNULL ( ( SELECT A.COL_DepartmentName FROM TBL_DepartmentMain A WHERE A.COL_DepartmentID = D.COL_DepartmentId  ) , '' ) As Col_DepartmentName ,
COUNT ( 1) OVER ( PARTITION BY A.COL_RegistrationId , A.COL_EncounterId ) As Col_InjectCount 
FROM TBL_ICMIndentMain A , TBL_ICMIndentDetails B , TBL_PhrItemMaster C , TBL_Employee D
WHERE A.COL_IndentId = B.COL_IndentId AND B.COL_ItemId = C.COL_ItemId AND A.COL_AdvisingDoctorId = D.COL_ID
AND  B.COL_RouteId >5
 "
